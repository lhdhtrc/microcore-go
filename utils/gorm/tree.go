...
func Tree[T interface{}](options TreeOptions) ([]*T, error) {
	...
	if !options.Lazy {
		...
		statement := fmt.Sprintf(`WITH RECURSIVE tree AS (SELECT * FROM %s WHERE app_id = '%s' %s UNION ALL SELECT t.* FROM %s t INNER JOIN tree ON tree.id = t.parent_id) SELECT * FROM tree;`, options.Table, options.AppId, parentId, options.Table)
